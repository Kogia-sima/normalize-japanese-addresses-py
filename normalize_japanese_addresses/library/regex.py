...
def getTownRegexes(pref: str, city: str, endpoint):
    def getChomeRegex(match_value: str):
        ...
        _regex = '|'.join(regexes)
        _regex = (
            f'({_regex})(([町丁])目?|番([町丁])|条|軒|線|の町?|地割|[-－﹣−‐⁃‑‒–—﹘―⎯⏤ーｰ─━])'
        )
    ...
    def towns_length(api_town):
        ...
        town_len = (
            town_len - 2 if str(api_town['town']).startswith('大字') else town_len
        )
        return town_len
    ...
    for town in api_pre_towns:
        ...
        if (
            townAddr not in api_towns_set
            and f'大字{townAddr}' not in api_towns_set
            and not isKanjiNumberFollewedByCho(  # 大字は省略されるため、大字〇〇と〇〇町がコンフリクトする。このケースを除外
                originalTown
            )
        ):
    ...
    for town in towns:
        ...
        for replace_town in re.finditer(
            ...
            _town = re.sub(
                replace_town.group(), getChomeRegex(replace_town.group()), _town
            )
...
def replace_addr(addr: str):
    ...
    addr = re.sub(
        '([0-9〇一二三四五六七八九十百千]+)(番地?)([(0-9〇一二三四五六七八九十百千]+)号?', '\\1-\\3', addr
    )
    ...
    for _find_addr in re.finditer(
        '([0-9〇一二三四五六七八九十百千]+)[-－﹣−‐⁃‑‒–—﹘―⎯⏤ーｰ─━]', addr
    ):
        _rp = re.sub('[-－﹣−‐⁃‑‒–—﹘―⎯⏤ーｰ─━]', '-', kan2num(_find_addr.group()))
        addr = addr.replace(_find_addr.group(), _rp)

    for _find_addr in re.finditer(
        '[-－﹣−‐⁃‑‒–—﹘―⎯⏤ーｰ─━]([0-9〇一二三四五六七八九十百千]+)', addr
    ):
        _rp = re.sub('[-－﹣−‐⁃‑‒–—﹘―⎯⏤ーｰ─━]', '-', kan2num(_find_addr.group()))
        addr = addr.replace(_find_addr.group(), _rp)
